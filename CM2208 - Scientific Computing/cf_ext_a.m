...
%   "s" with gain values specified in a vector variable named "b" for bands 
%   with central frequencies in "bands", type: ss = cf_equalise(s, b, bands)
%
%   Implementation:
%   The CF_EQUALISE function splits the sounds data from S{1} into bands
%   that center around the frequencies given in BANDS. A low shelf filter 
%   is applied to the first band, peak filters are applied to the middle 
%   bands and a high shelf filter is applied to the last band. These 
%   filters are given in __________. The gain values from B
